    ...
    gmail.observe.on("compose", (compose) => {
      gmail.tools.add_compose_button(
        ...
        async function () {
          ...
          const data = await get_email_data(compose);
      ...
      gmail.tools.add_compose_button(
        ...
        "Reponse Tu",
        async function () {
          const hook_url = get_hook_url("reponse_tu");
          const data = await get_email_data(compose);
      ...
      gmail.tools.add_compose_button(
        ...
        "Reponse Vous",
        async function () {
          const hook_url = get_hook_url("reponse_vous");
          const data = await get_email_data(compose);
          compose.body("Reponse en cours de calcul..." + compose.body());
          const result = await process_data(data, hook_url);
          compose.body(result);
      ...
      gmail.tools.add_compose_button(
        ...
        "Traduction FR to EN",
        async function () {
          const hook_url = get_hook_url("traduction_fr_to_en");
          const data = await get_email_data(compose);
...
async function get_email_data(compose) {
...
function prettify_text(text) {
  if (!text) return "";
  let pretty_text = text.replace(/<br>/g, "\n");
  pretty_text = pretty_text.replace(/<\/p>/g, "\n");
  pretty_text = pretty_text.replace(/<p>/g, "");
  return pretty_text;
}
